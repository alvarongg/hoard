...
class CatalogItem(UUIDMixin, TimestampMixin, Base):
    ...
    # Full-text search vector (PostgreSQL only, generated by a DB trigger).
    # Marked FetchedValue so the ORM never emits it in INSERT/UPDATE — the
    # column is a tsvector maintained entirely by the trigger.
